...
export default function IntensitySlider({ value, onChange }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 animate-fade-in">
      <h3 className="text-lg font-semibold text-gray-800 mb-2">Wie intensiv fühlt es sich an?</h3>
      ...
      <div className="space-y-4">
        ...
        {/* Aktueller Wert */}
        <div
          className={`p-4 rounded-xl ${value >= 3 ? 'bg-orange-50 border border-orange-200' : 'bg-gray-50 border border-gray-200'}`}
        >
          <div className="flex items-center gap-3">
            <div className={`w-4 h-4 rounded-full ${getColor(value)}`} />
            <div>
              <p className={`font-semibold ${getTextColor(value)}`}>{currentLevel.label}</p>
          ...
          {value >= 4 && (
            <div className="mt-3 p-3 bg-red-100 rounded-lg">
              <p className="text-sm text-red-700">
                <span className="font-medium">⚠️ Bei extremer Intensität:</span> Körperlich
                regulieren ist wichtig. Die TIPP-Skills können helfen, das Nervensystem schnell zu
                beruhigen.
              </p>
            </div>
          )}
