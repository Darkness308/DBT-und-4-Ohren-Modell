...
export default function SkillDetail({ skill, onBack, onMarkUsed }) {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      {/* Header */}
      <div
        className={`bg-gradient-to-r ${moduleColors[skill.module.id]} rounded-xl p-6 text-white`}
      >
        <button
          onClick={onBack}
      ...
      {skill.acronym && (
        <div className={`${moduleBgColors[skill.module.id]} rounded-xl p-6`}>
          <h3 className="font-semibold text-gray-800 mb-3">{skill.name} steht für:</h3>
          ...
          {skill.steps.map((step, index) => (
            <div key={index} className="flex gap-4">
              ...
              <div>
                ...
                {step.scienceNote && (
                  <p className="text-gray-500 text-xs mt-1 italic">ℹ️ {step.scienceNote}</p>
                )}
              </div>
            </div>
          ))}
        ...
        {!feedbackGiven ? (
          <>
            <p className="text-calm-700 mb-4">
              Hast du diesen Skill ausprobiert? Dein Feedback hilft dir, zu sehen, welche Skills für
              dich funktionieren.
            </p>
            {!showFeedback ? (
              <Button variant="calm" onClick={() => setShowFeedback(true)} icon="✓">
          ...
          <div className="flex items-center gap-3 text-green-700">
            ...
            <p className="font-medium">Super! Der Skill wurde in deiner History gespeichert.</p>
          </div>
        )}
      </div>

      {/* Zurück Button */}
      <Button variant="secondary" onClick={onBack} className="w-full" icon="←">
