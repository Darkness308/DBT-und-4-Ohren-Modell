...
import { vierOhrenDescriptions } from './vierOhrenExamples'
import RelatedSkills from '../../components/common/RelatedSkills'
...
export default function ResultDisplay({ analysis }) {
  ...
  const skillContext = [analysis.originalStatement, analysis.context, 'kommunikation']
    .filter(Boolean)
    .join(' ')
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
        ...
        <p className="text-lg font-medium text-gray-800">
          &ldquo;{analysis.originalStatement}&rdquo;
        </p>
        {analysis.context && (
  ...
  return (
    <div className={`bg-gradient-to-br ${colorClasses[description.color]} rounded-xl p-4 border`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className={`font-semibold ${textColors[description.color]}`}>{description.name}</h3>
      </div>

      <p className="text-xs text-gray-500 mb-2 italic">{description.question}</p>

      {/* Sachebene hat direkten Content */}
      {data.content && <p className="text-gray-700">{data.content}</p>}
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-4">
      <div className="flex items-center justify-between mb-2">
        <span className="text-sm font-medium text-gray-700">Potenzial für Missverständnisse</span>
        <span
          className={`text-sm font-medium ${value > 0.6 ? 'text-red-600' : value > 0.3 ? 'text-yellow-600' : 'text-green-600'}`}
        >
          {text} ({percentage}%)
        </span>
      </div>
      ...
      <p className="text-xs text-gray-500 mt-2">
        Je höher das Potenzial, desto wichtiger ist es, nachzufragen und die eigene Interpretation
        zu überprüfen.
      </p>
    </div>
  )
