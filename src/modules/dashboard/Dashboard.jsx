...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Skill-Nutzung Chart */}
      {skillHistory.length > 0 && <SkillUsageChart skillHistory={skillHistory} />}

      {/* Modul-Übersicht */}
      <ModuleOverview onNavigate={navigate} />

      {/* Letzte Aktivität */}
      {skillHistory.length > 0 && <RecentActivity skillHistory={skillHistory} />}

      {/* Hilfe-Hinweis */}
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
        <p className="text-blue-700 text-sm">
          <span className="font-medium">💡 Tipp:</span> Regelmäßiges Üben macht Skills zur
          Gewohnheit. Schon 5 Minuten täglich können einen Unterschied machen.
        </p>
      </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        <p className="text-gray-600 text-sm">
          <span className="font-medium">📞 Bei akuter Krise:</span> Telefonseelsorge{' '}
          <strong>0800 111 0 111</strong> (kostenlos, 24/7)
        </p>
      </div>
    </div>
  )
