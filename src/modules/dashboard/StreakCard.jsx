...
export default function StreakCard({ skillHistory }) {
  const stats = useMemo(() => {
    ...
    const lastUsed = sortedHistory[0]?.timestamp ? new Date(sortedHistory[0].timestamp) : null
  ...
  return (
    <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-6 border border-amber-200 animate-fade-in">
      ...
      <p className="text-sm text-gray-600 italic text-center">&ldquo;{message}&rdquo;</p>

      {/* Letzte Nutzung */}
      {stats.lastUsed && (
        <p className="text-xs text-gray-400 text-center mt-2">
          Zuletzt genutzt:{' '}
          {stats.lastUsed.toLocaleDateString('de-DE', {
            day: 'numeric',
            month: 'short',
            hour: '2-digit',
            minute: '2-digit',
