...
import { dbtModules } from '../dbt-skills/dbtSkills'
    ...
    <div className="bg-white rounded-xl shadow-md p-6 animate-fade-in">
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(value) => [`${value} mal`, 'Genutzt']} />
              </PieChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="h-48">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={effectivenessData} layout="vertical">
                ...
                <Tooltip formatter={(value) => [`${value} mal`, 'Bewertet']} />
                <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                  {effectivenessData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={
                        entry.rating >= 4 ? '#22c55e' : entry.rating >= 3 ? '#eab308' : '#ef4444'
                      }
                    />
                  ))}
                </Bar>
      ...
      {topSkills.length > 0 && (
        <div className="mt-6 pt-4 border-t border-gray-100">
          <h4 className="text-sm font-medium text-gray-600 mb-3">🏆 Meistgenutzte Skills</h4>
